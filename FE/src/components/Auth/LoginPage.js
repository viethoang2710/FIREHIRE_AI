...
const Login = () => {
  ...
  const validate = () => {
    ...
    if (validate()) {
      setLoading(true);
      try {
        // Use context login function instead of direct service call
        ...
        toast.error(error.message || 'Đăng nhập thất bại. Vui lòng kiểm tra lại thông tin đăng nhập.');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        <h2 className="auth-title">Log In to FireHire_AI</h2>
