      ...
      <aside className="w-full lg:w-[450px] bg-white p-6 overflow-y-auto shadow-xl z-10">
        ...
        <div className="space-y-3">
          ...
          <AccordionSection title="Kinh nghiệm làm việc" icon={Briefcase} isOpen={activeAccordion === 'experience'} onClick={() => setActiveAccordion('experience')}>
            {cvData.experience.map((exp) => (
              <div key={exp.id} className="p-3 border rounded-md mb-3 relative bg-gray-50">
                ...
                <input name="duration" value={exp.duration} onChange={(e) => handleListChange('experience', exp.id, e)} placeholder="Thời gian" className="input-field text-xs mb-2" />
                <textarea name="description" value={exp.description} onChange={(e) => handleListChange('experience', exp.id, e)} rows="4" placeholder="Mô tả công việc..." className="input-field text-sm"></textarea>
          ...
          <AccordionSection title="Học vấn" icon={Award} isOpen={activeAccordion === 'education'} onClick={() => setActiveAccordion('education')}>
            {cvData.education.map((edu) => (
              <div key={edu.id} className="p-3 border rounded-md mb-3 relative bg-gray-50">
                ...
                <input name="duration" value={edu.duration} onChange={(e) => handleListChange('education', edu.id, e)} placeholder="Thời gian" className="input-field text-xs" />
          ...
          <AccordionSection title="Kỹ năng" icon={Star} isOpen={activeAccordion === 'skills'} onClick={() => setActiveAccordion('skills')}>
            <p className="text-xs text-gray-500 mb-2">Liệt kê kỹ năng, cách nhau bởi dấu phẩy.</p>
      ...
      <main className="w-full flex-grow p-4 md:p-8 flex flex-col items-center justify-start lg:justify-center bg-gray-200">
        <div className="w-full max-w-[210mm] mb-4 p-2 bg-white rounded-lg shadow-md flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            }} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition-colors text-sm" title="Tải CV xuống">
              <Download size={16} className="mr-2" /> Tải xuống
            </button>
          </div>
        </div>

        <div ref={previewRef} className="w-full max-w-[210mm] max-h-[90vh] lg:max-h-[85vh] overflow-hidden rounded-lg shadow-2xl bg-white">
