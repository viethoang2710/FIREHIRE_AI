...
const ProfilePage = ({ showAlert }) => {
  ...
  const transformStatus = (apiStatus) => {
    ...
    return statusMap[apiStatus.toUpperCase()] || apiStatus.toLowerCase();
  };
  ...
  const loadApplications = async () => {
    try {
      ...
      console.log('🌐 API URL:', `http://localhost:8080/api/cv/user/${userId}`);
      
      const response = await fetch(`http://localhost:8080/api/cv/user/${userId}`, {
        method: 'GET',
        headers: {
      ...
      // API có thể trả về { success: boolean, data: [...] } hoặc trực tiếp array
      let cvData;
      if (data.success !== undefined) {
        cvData = data.data;
        console.log('📋 Found success property, using data.data:', cvData);
      ...
      if (cvData && Array.isArray(cvData) && cvData.length > 0) {
        console.log('✅ Tìm thấy', cvData.length, 'CV đã nộp');
        console.log('✅ First CV sample:', cvData[0]);
        
        // Chuyển đổi dữ liệu CV thật từ database (bảng CVs)
        const cvList = cvData.map((cv, index) => {
