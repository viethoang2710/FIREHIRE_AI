...
public interface JobPostingRepository extends JpaRepository<JobPosting, Integer> {
    ...
    // Find hot jobs (urgent or high view count)
    @Query("SELECT j FROM JobPosting j WHERE j.status = 'ACTIVE' AND (j.isUrgent = true OR j.viewCount > 100) ORDER BY j.viewCount DESC, j.createdDate DESC")
    List<JobPosting> findHotJobs(Pageable pageable);
